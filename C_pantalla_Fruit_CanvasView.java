...
public class C_pantalla_Fruit_CanvasView extends View {
    ...
    //Dibujamos los patos
    private static final int CANTIDADFRUIT = 8;
    ArrayList<C_pantalla_Fruit_Fruit1> fruit1; //Contendrá los patos de la primera clase Duck
    ...
    public C_pantalla_Fruit_CanvasView(Context context) {
        super(context);
        ...
        fruit1 = new ArrayList<>();
        //duck2 = new ArrayList<>();
        //duck3 = new ArrayList<>();
        for (int i=0; i<4; i++){//i = 4 porque creamos 4 patos
            C_pantalla_Fruit_Fruit1 Fruit_1= new C_pantalla_Fruit_Fruit1 (context);
            fruit1.add(Fruit_1);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        */
        ...
        for (int i=0; i<fruit1.size(); i++){
            canvas.drawBitmap(fruit1.get(i).getBitmap(), fruit1.get(i).posicionX, fruit1.get(i).posicionY, null);
            fruit1.get(i).punteroFruit++;
            //Cuando el punteroFruit llegue a cero
            if(fruit1.get(i).punteroFruit > (CANTIDADFRUIT -1 )){
                fruit1.get(i).punteroFruit = 0;
            }
            //La velocidad
            fruit1.get(i).posicionY -= fruit1.get(i).velocidad;


            if(fruit1.get(i).posicionY > (fruit1.get(i).getHeight())){
                fruit1.get(i).resetPosition();//Metodo para resetear la posición cuando llegue al borde de la pantalla
                puntos ++;
                BaseDeDatos();
    ...
    public void BaseDeDatos(){
        System.out.println("Entrando en el método BaseDeDatos Fruit");
        ...
        if (consulta.moveToFirst()) {//si hay datos
            ...
            if (puntos > bestScore) {
                ...
                BD.update("fruit", modificacion, "score =" + bestScore, null);
            }
